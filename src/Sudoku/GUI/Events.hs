...
handleEvents state@(State {stage="solver",dim=d,sudoku=su,..}) (MouseUp (mx, my))
  ...
  | isJust cell
  = (restoreState state { selectedCell = cell }, [GraphPrompt ("Ingrese Numero", hint)])
  where
    range = allowedChars su
    hint = "Rango (" ++ (show $ head range) ++ ".." ++ (show $ last range) ++ ")"
    cell = Raster.calculateCell mx my d
    f s = (s, redraw s $ MouseUp (mx, my))
    ns = step su [NS.resolveAllCandidates, HS.resolveAllCandidates, NP.resolveAllCandidates]

handleEvents state@(State {stage="solver",selectedCell=sc,sudoku=su,..}) (Prompt ("Ingrese Numero", n))
  = (s', redraw s' NoInput)
  where
