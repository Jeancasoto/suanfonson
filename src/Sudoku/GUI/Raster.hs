...
cellColor :: Color
cellColor = makeColor 0 0 0 1

gridLineWidth :: Float
gridLineWidth = 2.0

gridLineColor :: State -> Color
gridLineColor s | isValid (sudoku s) = Gfx.dim black
                | otherwise = black
...
drawCell (State {dim=d}) (MouseMotion (mx, my)) i j
  ...
  = Translate x y $ Pictures
    ...
    , Translate (-35 * scl) (-50 * scl) $ Color black $ Scale scl scl $ Text ""
    ]
  where
...
drawCell (State {invalidCell=sc,dim=d,sudoku=su}) _ i j
  = Translate (i' * cellSize) (j' * cellSize) $ Pictures $ [] ++
    (if isJust sc && fromJust sc == (d - j - 1, i) then
      [ Translate (-35 * scl) (-50 * scl) $ Color (black) $ Scale scl scl $ Text txt ]
    else
      [ Color cellColor $ rectangleWire cellSize cellSize
      , Translate (-35 * scl) (-50 * scl) $ Color (black) $ Scale scl scl $ Text txt
      ]
